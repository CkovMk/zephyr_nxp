...
static int imx93_display_mux_set(const struct device *dev)
{
	const struct imx93_ldb_config *cfg = dev->config;
	int err = 0;
	...
	case MEDIA_BUS_FMT_RGB666_1X7X3_SPWG:
		...
		LOG_INF("DISPLAY_MUX: RGB888_TO_RGB666 in %s", __func__);
		break;
	case MEDIA_BUS_FMT_RGB888_1X7X4_SPWG:
	case MEDIA_BUS_FMT_RGB888_1X7X4_JEIDA:
		cfg->base->GASKET.DISPLAY_MUX |= MEDIAMIX_BLK_CTRL_DISPLAY_MUX_PARALLEL_DISP_FORMAT(2);
		LOG_INF("DISPLAY_MUX: RGB888_TO_RGB888 in %s", __func__);
		break;
		default:
		err = -EINVAL;
	}
