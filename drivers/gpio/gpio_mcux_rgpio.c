				...
				gpio_pin_t pin, gpio_flags_t flags)
{
	...
	if (((flags & GPIO_PULL_UP) != 0) || ((flags & GPIO_PULL_DOWN) != 0)) {
		...
		/* Set pin to highz */
		reg &= ~((0x1 << MCUX_IMX_BIAS_PULL_DOWN_SHIFT) | (0x1 << MCUX_IMX_BIAS_PULL_UP_SHIFT));
	}
					      ...
					      enum gpio_int_trig trig)
{
	...
	irqs = 0; /* TODO: only irq0 is used for now */
...
static void mcux_rgpio_port_isr(const struct device *dev)
{
	...
	int_flags = base->ISFR[0]; /* TODO: only irq0 is used for now */
