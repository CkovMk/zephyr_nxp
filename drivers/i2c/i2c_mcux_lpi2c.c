...
static inline LPI2C_Type* mcux_lpi2c_get_base_addr(const struct device *dev)
{
#ifdef CONFIG_MMU
	return (LPI2C_Type*)DEVICE_MMIO_NAMED_GET(dev, reg_base);
#else /* CONFIG_MMU */
				...
				uint32_t dev_config_raw)
{
	...
	LPI2C_Type *base = mcux_lpi2c_get_base_addr(dev);
	uint32_t clock_freq;
	uint32_t baudrate;
	int ret;
...
#endif /* CONFIG_MMU */
	base = mcux_lpi2c_get_base_addr(dev);
